...
if [[ "$REDIS_VERSION" =~ ^[1-5][.] ]]; then # Redis 5 and earlier
    # Redis 5 and earlier do not support TLS
    # download
    ...
    (
        ...
        mkdir -p "$PREFIX"
        make install "-j$JOBS" PREFIX="$PREFIX" V=1
    )
    echo "::endgroup::"
else
    # bundle OpenSSL for better reproducibility.
    if [[ "$REDIS_VERSION" =~ ^6[.] ]]; then
        # Redis 6.x supports OpenSSL 1.1.1, but not OpenSSL 3.x
        echo "::group::download OpenSSL source"
        (
            set -eux
            cd "$RUNNER_TEMP"
            curl --retry 3 -sSL "https://github.com/openssl/openssl/archive/OpenSSL_$OPENSSL_VERSION1_1_1.tar.gz" -o openssl.tar.gz
        )
        echo "::endgroup::"
        ...
        (
            ...
            cd "$RUNNER_TEMP/openssl-OpenSSL_$OPENSSL_VERSION1_1_1"
        ...
        # Redis 7.x and later support OpenSSL 3.x
        echo "::group::download OpenSSL source"
        (
            set -eux
            cd "$RUNNER_TEMP"
            curl --retry 3 -sSL "https://github.com/openssl/openssl/archive/refs/tags/openssl-$OPENSSL_VERSION3.tar.gz" -o openssl.tar.gz
        )
        echo "::endgroup::"
        ...
        (
            set -eux
            cd "$RUNNER_TEMP/openssl-openssl-$OPENSSL_VERSION3"
